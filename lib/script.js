...
(function(window, screen) {
    ...
    function animate(yPos) {
        var path = svgElement.createPath(),
            container = svgElement.getContainer();

        path.attr('stroke', 'green').attr('stroke-width', '1').attr('fill', 'none');
        container.appendChild(path.el);
        var started = false;
        var d = '';
        var startWidth = 0;
        var endWidth = 0;
        var horizontalVariance = 0.04; //The less the value, less the variance
        var verticalVariance = 2; //The less the value, more the variance
        setInterval(function() {
            var randomNumber = startWidth + endWidth;
            var randomNumber2 = Math.random() * 20;
            if (!started) {
                d += 'M0 ' + (randomNumber);
                started = true;
            } else {
                d += 'L' + (randomNumber / horizontalVariance) + ' ' + (randomNumber2 / verticalVariance) + ' ';
            }
            path.attr('d', d);
            startWidth += 0.1;
            endWidth += 0.1;
